...
class SmsGatewayHandler(TwinRequestHandler):
    def do_GET(self):
        ...
        if parsed.path.startswith("/inbox/"):
            ...
            return self.send_json(200, {"ok": True, "msisdn": msisdn, "messages": msgs})
    ...
    def do_POST(self):
        ...
        to = data.get("to")
        if not to:
            return self.send_json(400, {"ok": False, "error": "to required"})
        with LOCK:
            _ID[0] += 1
        ...
        return self.send_json(200, {"ok": True, "id": msg["id"]})


if __name__ == "__main__":
    port = int(os.environ.get("PORT", "9810"))
    print(f"sms-gateway on :{port}", flush=True)
    ThreadingHTTPServer(("0.0.0.0", port), SmsGatewayHandler).serve_forever()
